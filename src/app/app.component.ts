...
@Component({
  ...
  template: `
    ...
    <a href="https://wa.me/919951028769?text=Hi%20Next%20Aura%20Communications,%20I'm%20interested%20in%20your%20services." 
...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    this.router.events.pipe(
      ...
      // Only scroll to top if the URL path changes (ignore Elfsight modifying history state)
      const currentUrlTree = this.router.parseUrl(this.router.url);
      const newUrlTree = this.router.parseUrl(event.urlAfterRedirects);
      
      const currentPath = currentUrlTree.root.children['primary']?.segments.join('/') || '';
      const newPath = newUrlTree.root.children['primary']?.segments.join('/') || '';
